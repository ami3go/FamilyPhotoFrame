...
(async () => {
  ...
  try {
    for (const [width,height] of viewports) {
      ...
      if(width===1440){const firstImage=await page.locator('#previewImage').elementHandle();await page.waitForTimeout(3250);const stable=await page.evaluate(image=>({same:image===document.getElementById('previewImage'),fetches:window.__previewFetches,revision:document.getElementById('previewImage').dataset.previewRevision}),firstImage);assert(stable.same,`${width}x${height}: status polling replaced the preview image DOM`);assert(stable.fetches===1,`${width}x${height}: unchanged preview was fetched ${stable.fetches} times`);assert(stable.revision==='preview-1',`${width}x${height}: preview revision was not applied`);}
